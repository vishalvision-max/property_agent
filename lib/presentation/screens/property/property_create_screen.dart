...
part 'property_create_screen_details.dart';

part 'property_create_screen_widgets.dart';

part 'property_create_screen_extras.dart';

part 'property_create_screen_basics.dart';
...
class _PropertyCreateScreenState extends ConsumerState<PropertyCreateScreen> {
  ...
  bool? _cafeteria;
  bool? _serverRoom;
  bool? _fireSafetyInstalled;
  bool? _centralAC;
  bool? _visitorParking;
  ...
  static const _shedTruckAccessOptions = <String>[
    'heavy_vehicle',
    'medium_vehicle',
    'small_vehicle',
  ];
  ...
  String _shedOwnership =
      ''; // freehold, leasehold, co-operative_society, power_of_attorney
  ...
  String _shedPossessionStatus =
      'ready_to_move'; // ready_to_move, under_construction
  ...
  bool? _pgAttachedBathroom;
  bool? _pgBalcony;
  final _pgRoomSize = TextEditingController();
  String _pgBedType = ''; // single, bunk
  bool? _pgCupboardAvailable;
  bool? _pgStudyTableAvailable;
  final _pgSecurityDeposit = TextEditingController();
  final _pgMaintenanceCharges = TextEditingController();
  bool? _pgElectricityIncluded;
  bool? _pgWaterIncluded;
  bool? _pgFoodChargesIncluded;
  bool? _pgBrokerageRequired;
  bool? _pgCoupleFriendly;
  bool? _pgIdProofRequired;
  ...
  bool? _pgSmokingAllowed;
  bool? _pgDrinkingAllowed;
  bool? _pgPetsAllowed;
  bool? _pgVisitorsAllowed;
  ...
  /// Hardcoded commercial subcategory slugs shown when the API returns no
  /// children for a Commercial parent.  Order matches the spec hierarchy.
  static const _commercialSubcategorySlugs = <String>[
    'office',
    'shop',
    'showroom',
    'warehouse',
  ...
  /// Human-readable display labels for the hardcoded commercial subcategories.
  static const _commercialSubcategoryLabels = <String, String>{
    'office': 'Office Space',
    'shop': 'Shop',
    'showroom': 'Showroom',
    'warehouse': 'Warehouse',
  ...
  /// Hardcoded Land/Plot subcategory slugs (Sale & Lease).
  static const _landPlotSubcategorySlugs = <String>[
    'residential_plot',
    'commercial_plot',
    'industrial_plot',
    'agricultural_land',
  ...
  /// Human-readable labels for the hardcoded Land/Plot subcategories.
  static const _landPlotSubcategoryLabels = <String, String>{
  ...
  // PG → Residential subcategories (all PG modes)
  ...
  // Co-Living → Residential subcategories
  static const _coLivingResidentialSubcategories = <String>[
    'co_living_space',
    'managed_co_living',
    'student_co_living',
    'working_professional_co_living',
    'luxury_co_living',
    'shared_apartment',
    'shared_villa',
    'shared_independent_house',
    'shared_builder_floor',
  ];

  // Sale → Residential allowed sub-slugs (keywords)
  static const _saleResidentialAllowedKeywords = <String>[
    'apartment',
    'flat',
    'builder',
    'floor',
  ...
  // Rent → Residential: slugs/keywords to EXCLUDE
  static const _rentResidentialExcludedKeywords = <String>[
    'pg',
    'hostel',
    'co-living',
    'coliving',
  ...
  // Lease → Residential allowed sub-slugs (keywords) — same as Sale
  static const _leaseResidentialAllowedKeywords = <String>[
    'apartment',
    'flat',
    'builder',
    'floor',
  ...
  // Sale → Commercial allowed sub-slugs (keywords)
  static const _saleCommercialAllowedKeywords = <String>[
    'office',
    'shop',
    'showroom',
    'warehouse',
  ...
  // Rent/Lease → Commercial allowed sub-slugs (keywords)
  static const _rentLeaseCommercialAllowedKeywords = <String>[
    'office',
    'shop',
    'showroom',
    'warehouse',
  ...
  // Sale/Lease → Land/Plot allowed sub-slugs (keywords)
  static const _saleLeaseLandAllowedKeywords = <String>[
    'residential',
    'commercial',
    'industrial',
    'agricultural',
  ...
  void _syncDetailsFromSelectedCategorySlugs({
    bool resetCaseSpecificSelections = true,
  }) {
    if (resetCaseSpecificSelections) {
  ...
  void _prefillFromProperty(Property p) {
    ...
    final residential = Map<String, dynamic>.from(
      f['residential_details'] is Map
          ? f['residential_details'] as Map
    ...
    if (_selectedAmenityIds.isEmpty) {
      final amenityIds = _fl(f, ['amenity_ids']);
      if (amenityIds.isNotEmpty) {
        _selectedAmenityIds.addAll(
          amenityIds.map((e) => int.tryParse(e)).whereType<int>(),
        );
      } else {
    ...
    if (_furnishing.isEmpty) {
      _furnishing = _f(residential, ['furnishing']) ??
          _f(plot, ['furnishing']) ??
          _f(plot, ['furnishing_type']) ??
          _f(plot, ['furnishing_status']) ??
          _f(plot, ['furnished']) ??
          _furnishing;
    }
    // ───────────── PG PREFILL ─────────────
    ...
    _pgFoodAvailability
      ..clear()
      ..addAll(_fl(f, ['pg_food_availability']));
    final _legacyFood =
    ...
    _pgSecurity =
        _fbNullable(f, ['pg_security']) ?? _fbNullable(pg, ['pg_security']);
    ...
    _length.text = p.plotLength?.toString() ??
        ...
        _fd(plot, ['plot_length_ft', 'plot_length'])?.toString() ??
        _fd(plot, ['plot_length', 'plot_length_ft'])?.toString() ??
        '';
    _breadth.text = p.plotBreadth?.toString() ??
        ...
        '';
    _floorsAllowed.text = p.floorsAllowed?.toString() ??
        _fi(f, ['floors_allowed'])?.toString() ??
        _fi(plot, ['floors_allowed'])?.toString() ??
        '';
    _openSides = p.openSides ??
        _fi(f, ['open_sides']) ??
        _fi(plot, ['open_sides']) ??
        _openSides;
    _boundaryWall = p.boundaryWall ??
        _fb(f, ['boundary_wall']) ??
        _fbNullable(plot, ['boundary_wall']);
    _constructionDone = p.constructionDone ??
        _fb(f, ['construction_done']) ??
        _fbNullable(plot, ['construction_done']);
    ...
    if (_readyTimeframe.isEmpty && p.propertyAge != null) {
      final years = p.propertyAge!;
      if (years <= 1) {
        _readyTimeframe = '0_1';
      } else if (years <= 5) {
        _readyTimeframe = '1_5';
    ...
    _additionalRooms
      ..clear()
      ..addAll(
        (p.additionalRooms ?? _fl(f, ['additional_rooms']))
            .map((r) => r == 'servent_room' ? 'servant_room' : r),
      );
    if (_additionalRooms.isEmpty) {
      _additionalRooms.addAll(
        _fl(residential, ['additional_rooms'])
            .map((r) => r == 'servent_room' ? 'servant_room' : r),
      );
    }
    ...
    if (_propertyHighlights.isEmpty) {
      _propertyHighlights.addAll(_fl(residential, ['nearby']));
    }
    if (_propertyHighlights.isEmpty) {
      _propertyHighlights.addAll(_fl(residential, ['property_highlights']));
    }
    _whatsappUpdates = _fb(f, ['whatsapp_updates'], fallback: true);
    _promotionTags
      ..clear()
      ..addAll(_fl(f, ['promotion']));
    if (_promotionTags.isEmpty) {
      _promotionTags.addAll(_fl(residential, ['promotion_tags']));
    }
    ...
    if (_rentAdditionalRooms.isEmpty) {
      _rentAdditionalRooms.addAll(_fl(residential, ['additional_rooms']));
    }
    _rentCornerProperty =
        p.rentCornerProperty ?? _fbNullable(f, ['rent_corner_property']);
    _petFriendly = _fbNullable(f, ['pet_friendly']);
    ...
    if (_rentTenantTypes.isEmpty) {
      _rentTenantTypes.addAll(_fl(residential, ['tenant_types']));
    }
    ...
    _villaAdditionalRooms
      ..clear()
      ..addAll(
        (p.additionalRooms ?? _fl(f, ['villa_additional_rooms']))
            .map((r) => r == 'servent_room' ? 'servant_room' : r),
      );
    if (_villaAdditionalRooms.isEmpty) {
      _villaAdditionalRooms.addAll(
        _fl(residential, ['additional_rooms'])
            .map((r) => r == 'servent_room' ? 'servant_room' : r),
      );
    }
    ...
    if (_duplexNearbyFacilities.isEmpty) {
      _duplexNearbyFacilities.addAll(_fl(plot, ['nearby_facilities']));
    }
    _maintenanceCharges.text = p.maintenanceCharges?.toString() ??
        _fd(f, ['maintenance_charges'])?.toString() ??
        '';
    ...
    if (_farmUtilities.isEmpty) {
      _farmUtilities.addAll(_fl(plot, ['farm_utilities']));
    }
    ...
    if (foodVal != null || foodPref != null) {
      final norm = normalizeFood(foodVal, foodPref);
      _pgFoodAvailability.clear();
      if (norm.isNotEmpty) _pgFoodAvailability.add(norm);
    }
  ...
  Future<void> _saveDraft() async {
    try {
      ...
      final data = <String, Object?>{
        ...
        'pgFoodAvailability': _pgFoodAvailability.toList(growable: false),
  ...
  void _applyDraft(Map<String, dynamic> decoded) {
    setState(() {
      ...
      _shedSuperBuiltUpArea.text =
          (decoded['shedSuperBuiltUpArea'] as String?) ?? '';
      ...
      _shedPropertyAge =
          (decoded['shedPropertyAge'] as String?) ?? _shedPropertyAge;
      _shedFacing = (decoded['shedFacing'] as String?) ?? _shedFacing;
      _shedCeilingHeight.text = (decoded['shedCeilingHeight'] as String?) ?? '';
      _shedFloorLoadCapacity.text =
          (decoded['shedFloorLoadCapacity'] as String?) ?? '';
      ...
      _shedBorewellAvailable =
          (decoded['shedBorewellAvailable'] as bool?) ?? _shedBorewellAvailable;
      _shedLoadingBay = (decoded['shedLoadingBay'] as bool?) ?? _shedLoadingBay;
      _shedDockLeveler =
          (decoded['shedDockLeveler'] as bool?) ?? _shedDockLeveler;
      _shedTruckAccess =
          (decoded['shedTruckAccess'] as String?) ?? _shedTruckAccess;
      _shedContainerAccess =
          (decoded['shedContainerAccess'] as bool?) ?? _shedContainerAccess;
      _shedParkingAvailable =
          (decoded['shedParkingAvailable'] as bool?) ?? _shedParkingAvailable;
      _shedVisitorParking =
          (decoded['shedVisitorParking'] as bool?) ?? _shedVisitorParking;
      _shedSecurityCabin =
          (decoded['shedSecurityCabin'] as bool?) ?? _shedSecurityCabin;
      _shedBoundaryWall =
          (decoded['shedBoundaryWall'] as bool?) ?? _shedBoundaryWall;
      ...
      _shedMaintenanceCharges.text =
          (decoded['shedMaintenanceCharges'] as String?) ?? '';
      _shedPriceNegotiable =
          (decoded['shedPriceNegotiable'] as bool?) ?? _shedPriceNegotiable;
      _shedPossessionStatus =
          (decoded['shedPossessionStatus'] as String?) ?? _shedPossessionStatus;
      _shedSecurityDeposit.text =
          (decoded['shedSecurityDeposit'] as String?) ?? '';
      ...
      _shedNoticePeriodValue.text =
          (decoded['shedNoticePeriodValue'] as String?) ?? '';
      _shedRentNegotiable =
          (decoded['shedRentNegotiable'] as bool?) ?? _shedRentNegotiable;
      _shedLeaseDuration.text = (decoded['shedLeaseDuration'] as String?) ?? '';
      _shedEscalationClause =
          (decoded['shedEscalationClause'] as bool?) ?? _shedEscalationClause;
      _shedRenewalOption =
          (decoded['shedRenewalOption'] as bool?) ?? _shedRenewalOption;
      ...
      _pgFoodAvailability
        ..clear()
        ..addAll(
          ((decoded['pgFoodAvailability'] as List?)?.cast<String>() ?? []),
        );
      _pgPropertyType =
  ...
  void _validateField(String field) {
    ...
    switch (field) {
      ...
      case 'area':
        ...
        break;
      case 'desc':
        err = Validators.minLen(_description.text, 15, label: 'Description');
        break;
      case 'address':
        err = Validators.requiredText(_address.text, label: 'Address');
        break;
      case 'city':
  ...
  Future<void> _create() async {
    ...
    final property = Property(
      ...
      area: area,
      areaUnit: _areaUnit,
      carpetArea: double.tryParse(_carpetArea.text.trim()),
      superBuiltUpArea: double.tryParse(_superBuiltUpArea.text.trim()),
    ...
    debugPrint(
      '[PropertyCreateScreen] fullPayload=${property.toJson()}',
    );
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      ...
      if (targetKey != null && targetKey.currentContext != null) {
        Scrollable.ensureVisible(
          ...
          alignment:
              0.02, // anchor the top of the section to the top of the screen
        );
      }
    });
  }
